...
def _color_bar(css_class: str) -> str:
    """Map category color-bar class to a hex color."""
    colors = {
        "cb-navy": "#1B2A4A",
        "cb-red": "#C0392B",
        "cb-lt": "#2980B9",
...
def _signal_badge(signal_type: str) -> str:
    """Return an inline-styled badge for signal type."""
    badge_colors = {
        "ESCALATION": "#C0392B",
        "ANOMALY": "#8E44AD",
        "DEVELOPMENT": "#2980B9",
...
def _social_badge(badge_class: str) -> str:
    """Map social badge class to colors."""
...
def render(digest: dict) -> str:
    ...
    if kcna:
        ...
        us_tone = _esc(kcna.get("us_tone", "—"))
        rok_tone = _esc(kcna.get("rok_tone", "—"))
        russia_tone = _esc(kcna.get("russia_tone", "—"))
        china_tone = _esc(kcna.get("china_tone", "—"))
        ...
        phrases_html = ""
        if key_phrases:
            phrases_html = "<div style='margin-top:8px;font-size:12px;color:#555;'><strong>New phrases:</strong> " + ", ".join(_esc(str(p)) for p in key_phrases) + "</div>"

        sections.append(f"""
        <div style="padding:24px 32px;background:#FDF6E3;border-bottom:1px solid #E0E0E0;">
          <h2 style="margin:0 0 12px 0;font-size:14px;color:#1B2A4A;text-transform:uppercase;letter-spacing:1px;">
            KCNA Delta {"⚠" if watch else ""}
          </h2>
          <div style="display:flex;gap:16px;margin-bottom:12px;font-size:12px;">
            <div><strong>Kim today:</strong> {kim_today}</div>
            <div><strong>US:</strong> {us_tone}</div>
            <div><strong>ROK:</strong> {rok_tone}</div>
            <div><strong>Russia:</strong> {russia_tone}</div>
            <div><strong>China:</strong> {china_tone}</div>
          </div>
    ...
    # ── Friday Memo ───────────────────────────────────────────────────────
    friday_memo = digest.get("friday_memo")
    if friday_memo:
        sections.append(f"""
        <div style="padding:24px 32px;background:#1B2A4A;color:#fff;border-bottom:1px solid #E0E0E0;">
          <h2 style="margin:0 0 12px 0;font-size:14px;text-transform:uppercase;letter-spacing:1px;color:#D4AC0D;">
            Friday Analytical Memo
          </h2>
          <p style="margin:0;font-size:14px;line-height:1.7;font-family:'Georgia',serif;color:#E8E8E8;">
            {_esc(friday_memo)}
          </p>
        </div>
        """)
