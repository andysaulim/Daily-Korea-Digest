...
- K-POP / ENTERTAINMENT: Do NOT include K-pop, BTS, Blackpink, K-drama, entertainment, or celebrity news in ANY section. These are not policy-relevant. Do NOT use "K-pop" or "Entertainment" as a category.
...
- overnight_items: 6-9 highest-priority items. Each: url, source, category, headline (under 100 chars), body_text (2-3 sentences — summarize the key facts, then add context or implication. Be substantive — thin one-liners waste the reader's time)
- top_stories: 2-4 biggest HARD NEWS stories of the day — include 3 on a typical day, 2 on slow days, 4 when multiple major stories compete. the stories generating the most noise, traction, and attention in Korea policy circles. These must be original reporting from wire services (Reuters, AP, AFP), correspondents (WSJ, NYT, WaPo, FT), Korean dailies (Yonhap, Korea Herald, Chosun, JoongAng), or government sources — NOT op-eds, analysis, think tank commentary, or publications like The Diplomat, Foreign Affairs, Brookings, etc. Pick the stories a Korea desk officer would be briefing their boss on first thing in the morning. Each: url, source, category_tag (use only: DPRK, US-Korea, NK-Russia-China, Technology, Business, Energy, Japan-Korea, China-Korea, Trilateral — do NOT use "Security" or "ROK Policy"), headline, body (2-3 sentences — lead with the key facts: who, what, when, specific numbers. Add one beat of factual context: what changed vs. yesterday, what's new. Do NOT interpret — state facts), so_what (1 sentence — name the specific decision, meeting, or timeline this directly affects. No editorializing. Example: "On the agenda for the Mar 28 Quad foreign ministers meeting"), pattern_note (1 sentence citing a specific historical precedent with dates, if applicable), src_line
...
- us_korea_deals: US-Korea trade and investment deals. Object with four keys:
  ...
  - trade_policy: array of 5-8 items tracking US trade policy actions specifically targeting or affecting South Korea under the CURRENT Trump administration. Focus on: USTR Section 301 investigation into ROK, Department of Commerce tariff determinations, USTR tariff schedules affecting Korean goods, Commerce Dept export controls on Korea-bound technology, CFIUS reviews of Korean acquisitions, ITC trade remedy cases involving Korean companies, bilateral trade negotiation rounds. Each: item (short name, e.g. "Section 301 Investigation", "Auto Tariff (25%)", "Steel/Aluminum Tariff", "Reciprocal Tariff", "Semiconductor Export Controls"), agency (which US agency — "USTR", "Commerce Dept", "CFIUS", "ITC", "Treasury"), detail (1 sentence on current status — be specific about rates, dates, deadlines), status (one of: ACTIVE, PENDING, RISK, ESCALATION, RESOLVED, MONITOR). IMPORTANT: Only include policies that are currently active or relevant under the Trump administration. Do NOT include "Critical Minerals Agreement" (remove it). For CHIPS Act and IRA EV Credit: only include these if they are still actively being implemented — note that the Trump administration has rolled back or paused many Biden-era programs, so check current status and mark as RESOLVED or PAUSED if applicable. Update statuses based on today's news. Always include this array with at least 5 items reflecting the current US trade policy landscape toward South Korea.
  - tariff_tracker: current US tariff rates on South Korean goods. Object with:
    - headline_rate: string — the main reciprocal tariff rate (e.g. "25%")
    - headline_status: one of "ACTIVE", "PAUSED", "NEGOTIATING", "ESCALATION", "REDUCED" — current status of the headline tariff
    - headline_note: 1 sentence on current status/context (e.g. "Trump raised from 15% to 25% on Jan 27 after ROK parliament failed to ratify trade pact; parliament approved in mid-March")
    - sector_rates: array of sector-specific tariff objects, each with: sector (e.g. "Steel & Aluminum", "Automobiles", "Semiconductors"), rate (e.g. "50%", "25%"), authority (e.g. "Section 232", "Section 301", "Section 122"), status ("ACTIVE"/"PAUSED"/"PENDING"/"EXEMPT"), note (1 sentence on current status)
    - section_122_surcharge: string — current Section 122 global surcharge rate if active (e.g. "10%"), or null
    - last_change: string — date and description of most recent tariff change (e.g. "Jan 27, 2026: Trump raised reciprocal rate from 15% to 25%")
    - next_trigger: string or null — upcoming deadline or event that could change rates (e.g. "USTR 90-day review due Apr 15", "Supreme Court ruling pending")
    Update these values based on today's news. If no tariff news today, carry forward the most accurate known values.
...
- public_sentiment: standing dashboard of Korean public opinion polling — ALL metrics MUST come from the SAME Gallup Korea weekly poll (same survey date). Do NOT mix dates across metrics. Object with:
  - presidential_approval: object with value (percentage as string, e.g. "62%"), trend (up/down/stable), source (polling firm name, e.g. "Gallup Korea"), last_updated (date string, e.g. "Dec 2025"). IMPORTANT: Use the LATEST Gallup Korea weekly poll. As of December 2025 the latest confirmed figures are: 62% approval, DP 43%, PPP 22%, independents 28%. Update ALL metrics together if new polling data appears in today's articles, otherwise carry forward the baseline values provided. Never mix poll dates.
...
- ENTERTAINMENT FILTER: Do NOT include K-pop (BTS, Blackpink, etc.), K-drama, celebrity, or entertainment news in any section. These are not policy-relevant for this briefing's audience.
