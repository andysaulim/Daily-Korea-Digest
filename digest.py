...
- SAME TOPIC from multiple sources = ONE entry only. If Reuters, Yonhap, and Korea Herald all cover the same event, include the BEST source once. Do NOT include multiple entries about the same event, meeting, or announcement. This applies WITHIN sections too — never place two articles about the same subject in top_stories (or any other section). Different angles on the same topic (e.g. "BOK nominee announced" and "BOK nominee's policy views") are the SAME topic — pick the best, move the other to a lower section.
