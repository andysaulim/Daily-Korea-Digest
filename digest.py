...
- us_korea_deals: US-Korea trade and investment deals. Object with three keys:
  - investment_package: running status of the ROK-US $350B investment commitment. Object with: total_pledged (string, e.g. "$350B"), announced_to_date (string — cumulative amount announced so far based on known WH tracker entries below plus any new deals today, e.g. "$87.5B"), pct_fulfilled (integer 0-100 — announced_to_date / total_pledged as percentage), latest_update (string — 1 sentence on the most recent deal or change that moved the total). Calculate announced_to_date by summing all known WH tracker entries below plus any new deals from today's news. Always include this object.
  - status_tracker: array of 5-8 key US-Korea policy/agreement items to monitor. Each: item (short name, e.g. "25% Auto Tariff", "Semiconductor MOU", "CHIPS Act Waiver", "SMA Cost-Sharing", "Nuclear 123 Agreement", "Critical Minerals FTA", "EV Tax Credit (IRA §30D)", "Steel/Aluminum Tariff"), detail (1 sentence on current status), status (one of: ACTIVE, PASSED, RISK, MONITOR, PRESSURE). Update statuses based on today's news — if a tariff was imposed, mark ACTIVE; if a waiver is at risk, mark RISK; etc. Always include this array with at least 5 items reflecting current policy landscape.
  ...
  REFERENCE — White House Investment Tracker (whitehouse.gov/investments). The ROK-US investment deal was announced as $350B at the Aug 2025 summit. Known Korea entries on the WH tracker (use these to calculate announced_to_date):
    South Korea (country-level): $350B commitment (Aug 2025 summit)
...
- japan_korea: array of 2-3 items covering Japan-Korea bilateral and US-ROK-Japan trilateral developments from today's news. Each: url, source, headline, body_text (1-2 sentences — facts first, then one beat of context), category (one of: history-dispute, trilateral, gsomia, trade-friction, diplomatic, defense-cooperation, territorial), signal_type (ESCALATION/ANOMALY/DEVELOPMENT/CONFIRMATION/CONTEXT). Include items categorized as Japan-Korea or Trilateral. Empty array if no relevant stories today. Do NOT duplicate items already in top_stories or overnight_items.
- china_korea: array of 2-3 items covering China-Korea relations and PRC influence/pressure on ROK from today's news. Each: url, source, headline, body_text (1-2 sentences), category (one of: thaad-retaliation, economic-coercion, rare-earth, diplomatic, military, trade, public-opinion), signal_type, is_reaction_source (boolean — true if from Global Times, Xinhua, People's Daily, China Daily). Empty array if no relevant stories today. Do NOT duplicate items already in top_stories or overnight_items.
- public_sentiment: standing dashboard of Korean public opinion. Object with:
  - presidential_approval: object with value (percentage as string, e.g. "23%"), trend (up/down/stable), source (polling firm name), last_updated (date string, e.g. "Mar 21, 2026"). Update if new polling data appears in today's articles, otherwise carry forward best known values or use null for value.
  - favorability_us: object with value (percentage as string), source (e.g. "Asan Institute"), last_updated. Korean public favorability toward the United States.
  - favorability_china: object with value (percentage as string), source, last_updated. Korean public favorability toward China.
  - favorability_japan: object with value (percentage as string), source, last_updated. Korean public favorability toward Japan.
...
IMPORTANT — NO OVERLAP: Each article must appear in exactly ONE section. A story in top_stories must NOT also appear in overnight_items, also_today, japan_korea, china_korea, or us_korea_deals. Deduplicate by URL — if a story qualifies for multiple sections, place it in the highest-priority section: top_stories > overnight_items > japan_korea > china_korea > us_korea_deals > also_today.
