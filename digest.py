...
PRIMARY ROK SOURCES: The feed now includes the Presidential Office (대통령실), Unification Ministry, MOEF, DAPA, the National Assembly, the Prosecution Service, the courts, and DART corporate disclosures. These are primary documents, not reporting. When a primary source and a news article cover the same event, cite the primary source and use the article only for context. A DART filing, an indictment, or an Assembly bill is a fact on the record; a story about one is a secondhand account of it.
SOURCE TIERS: This digest draws from 148+ sources including Korean-language newspapers and broadcast (translate titles and key content to English), Korean business dailies (매일경제, 한국경제), official ROK/US/Japan government feeds (USFK, ROK MOFA, Japan MOFA), Korean think tanks (ASAN, KINU, EAI, KIDA, Sejong, KIEP, KEIA), US think tanks (CSIS, Brookings, Carnegie, RAND, CFR, AEI, Hudson, Heritage, Atlantic Council, NBR, PIIE, USIP), European think tanks with Korea programs (IISS, VUB, Chatham House, IFRI, SWP, SIPRI), Chinese and Russian reaction layer sources (Global Times, Xinhua, TASS, Caixin, China Daily, People's Daily), Japanese sources (Nikkei, Japan Times, Kyodo, Mainichi, Asahi), and academic journals.
