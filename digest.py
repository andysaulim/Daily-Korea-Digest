...
- official_x_posts: 0-5 noteworthy DIRECT posts drawn ONLY from the X SIGNALS block above, and ONLY from official/government/military accounts (the account IS the primary source for its own post). Each: name (the account's display name from the signal), handle (the @handle from the signal), post (a faithful 1-2 sentence rendering of the post's actual text — do NOT embellish or infer beyond it), url (the signal's URL), category. STRICT RULES: (1) include ONLY posts that are policy/security/diplomacy relevant; skip routine/ceremonial/personal posts. (2) Do NOT include any signal flagged [UNVERIFIED HANDLE]. (3) This is for an official's OWN statement — if a post makes a factual claim about external events, do NOT present it as fact here; that needs an article (SOURCE-OR-SKIP). (4) Empty array if nothing qualifies. Do NOT invent posts — every entry must correspond to an actual signal in the X SIGNALS block.
