...
DPRK SPECIALIST RULE — MANDATORY INCLUSION: If ANY article appears from 38 North, AccessDPRK, or ArmsControlWonk, it MUST be included in the digest — in also_today (The Wire) by default, or top_stories/overnight_items if the finding is major (e.g. new satellite imagery, nuclear/missile test indicators, significant policy analysis). These are the gold-standard DPRK-focused sources; they publish infrequently, and when they do it is always substantive. Never drop an article from these three sources.
...
- us_korea_deals: US-Korea trade and investment deals. Object with four keys. IMPORTANT — NO REPETITION across sub-sections: tariff rates belong ONLY in tariff_tracker (do not repeat rates in trade_policy). Investment totals belong ONLY in investment_package (do not restate in deals). trade_policy covers non-tariff policy actions only. Each fact appears exactly once.
  ...
  - trade_policy: array of 4-6 NON-TARIFF US trade policy actions (MAX 6) affecting South Korea. Do NOT repeat tariff rates already shown in tariff_tracker. Focus on: Section 301 investigations, export controls, CFIUS reviews, ITC cases, trade negotiation rounds. Each: item, agency, detail (1 sentence — current status with dates/deadlines), status (ACTIVE/PENDING/RISK/ESCALATION/RESOLVED/MONITOR), url (link to today's source article or the most recent authoritative source for this policy action — e.g. Federal Register notice, USTR announcement, Reuters/AP report. REQUIRED for every item). Only include currently active/relevant policies.
    ...
    Samsung and SK Hynix China fab licenses are EXPORT CONTROL licenses (BIS), NOT "CHIPS Act" provisions. As of Jan 2026 these shifted from indefinite VEU waivers to restrictive ANNUAL export licenses (maintenance only, no expansion). Do NOT describe these as "CHIPS Act exclusions" or "CHIPS restrictions." Use "BIS export licenses" or "semiconductor export controls."
