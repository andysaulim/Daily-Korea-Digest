...
- top_stories: exactly 3 biggest HARD NEWS stories of the day (ALWAYS include 3 — never fewer) — the stories generating the most noise, traction, and attention in Korea policy circles. These must be original reporting from wire services (Reuters, AP, AFP), correspondents (WSJ, NYT, WaPo, FT), Korean dailies (Yonhap, Korea Herald, Chosun, JoongAng), or government sources — NOT op-eds, analysis, think tank commentary, or publications like The Diplomat, Foreign Affairs, Brookings, etc. Pick the stories a Korea desk officer would be briefing their boss on first thing in the morning. Each: url, source, category_tag, headline, body (2-3 sentences — lead with the key facts: who, what, when, specific numbers. Add one beat of factual context: what changed vs. yesterday, what's new. Do NOT interpret — state facts), so_what (1 sentence — name the specific decision, meeting, or timeline this directly affects. No editorializing. Example: "On the agenda for the Mar 28 Quad foreign ministers meeting"), pattern_note (1 sentence citing a specific historical precedent with dates, if applicable), src_line
...
- public_sentiment: standing dashboard of Korean public opinion polling — ALL metrics MUST come from the SAME Gallup Korea weekly poll (same survey date). Do NOT mix dates across metrics. Object with:
  - presidential_approval: object with value (percentage as string, e.g. "67%"), trend (up/down/stable), source (polling firm name, e.g. "Gallup Korea"), last_updated (date string, e.g. "Mar 17-19, 2026"). IMPORTANT: Use the LATEST Gallup Korea weekly poll. As of March 2026 the latest figures are: 67% approval, DP 46%, PPP 20%, independents 27% (Mar 3rd week, surveyed Mar 17-19). Update ALL metrics together if new polling data appears in today's articles, otherwise carry forward the baseline values provided. Never mix poll dates.
