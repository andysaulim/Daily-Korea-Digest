...
- calendar_watch: array of 5 key upcoming events in the next 14-30 days relevant to Korea policy (MAX 5 — pick the most consequential). GROUNDING: Only include events that are (a) mentioned in today's source articles with a specific date, (b) in the VERIFIED UPCOMING DATES reference below, or (c) in the tariff/trade baseline deadlines in this prompt. Do NOT invent upcoming events or dates from memory. You MAY repeat an entry from a previous digest if the event is still upcoming — calendar items should persist until their date passes. Each: month (3-letter, e.g. "MAR", "APR"), day (integer), headline (short title), detail (1-2 sentences). No signals, no urgency labels — just the event and when it happens.
