...
- calendar_watch: array of 4-5 key upcoming events in the next 14-30 days relevant to Korea policy (MINIMUM 4, MAX 5 — always fill this section). GROUNDING: Only include events that are (a) mentioned in today's source articles with a specific date, (b) in the VERIFIED UPCOMING DATES reference below, or (c) in the tariff/trade baseline deadlines in this prompt. Do NOT invent upcoming events or dates from memory. You MAY repeat an entry from a previous digest if the event is still upcoming — calendar items should persist until their date passes. Each: month (3-letter, e.g. "MAR", "APR"), day (integer), headline (short title), detail (1-2 sentences). No signals, no urgency labels — just the event and when it happens.
  ...
  Apr 14 2026: Commerce/USTR semiconductor tariff report deadline — determines broader chip tariff rates beyond narrow 25% advanced logic tariff.
  Apr 15: Kim Il Sung birthday (Day of the Sun) — DPRK's most important national holiday; watch for provocations in the lead-up.
  Apr 24 2026: BOK Monetary Policy Board meeting — rate decision.
  ...
  Jun 4 2026: ROK local elections — governors, mayors, provincial/city council seats nationwide. First electoral test for Lee Jae-myung administration.
...
- public_sentiment: standing dashboard of Korean public opinion polling — ALL metrics MUST come from the SAME Gallup Korea weekly poll (same survey date). Do NOT mix dates across metrics. Object with:
  - presidential_approval: object with value (percentage as string, e.g. "67%"), trend (up/down/stable), source (polling firm name, e.g. "Gallup Korea"), last_updated (date string, e.g. "Mar 17-19, 2026"). IMPORTANT — HARD BASELINE: As of April 5th week 2026 the latest confirmed Gallup Korea figures are: 64% approval, DP 46%, PPP 21%, independents 27% (surveyed Apr 28-30). Use these EXACT numbers. Do NOT change ANY polling number unless a TODAY's source article explicitly reports a NEW Gallup Korea weekly poll with different figures AND you can cite the specific article. Changing a polling number without a sourced article is a fabrication. When in doubt, carry forward 64%/46%/21%/27%. Never mix poll dates.
